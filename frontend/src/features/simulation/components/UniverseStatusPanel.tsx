...
import { useUniverseMetrics } from '@/features/universe';
import { useToggleUniverse, useDeleteUniverse } from '../hooks';
...
export default function UniverseStatusPanel({
  ...
  const { metrics } = useUniverseMetrics(universeId);
  const toggleMutation = useToggleUniverse();
  const deleteMutation = useDeleteUniverse();

  const [showDeleteModal, setShowDeleteModal] = useState(false);

  const status = metrics?.status ?? universe?.status ?? 'unknown';
  const isActive = status === 'active';

  const handleToggle = () => {
    if (!universeId) return;
    toggleMutation.mutate(universeId);
  };

  const handleDelete = () => {
    if (!universeId) return;
    deleteMutation.mutate(universeId, {
      onSuccess: () => {
    ...
    <>
      <SectionPanel>
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={toggleMutation.isPending || !universeId}
        ...
        <div className="space-y-0 divide-y divide-slate-800/60">
          {[
            { label: 'Tick', value: String(metrics?.current_tick ?? universe?.current_tick ?? 0), mono: true },
